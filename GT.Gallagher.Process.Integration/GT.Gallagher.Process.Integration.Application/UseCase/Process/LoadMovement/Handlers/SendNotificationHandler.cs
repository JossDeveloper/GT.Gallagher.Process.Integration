...
public class SendNotificationHandler : Handler<ProcessLoadMovementRequest>
{
    ...
    private readonly IProcessLoadMovementRepository processLoadMovementRepository;

    public SendNotificationHandler(ITPAServiceTokenRepository tokenRepository, INotificationEndProcessRepository notificationRepository, IProcessLoadMovementRepository processLoadMovementRepository)
    {
        this.tokenRepository = tokenRepository;
        this.notificationRepository = notificationRepository;
        this.processLoadMovementRepository = processLoadMovementRepository;
    }
    ...
    public override async Task ProcessRequest(ProcessLoadMovementRequest request)
    {
        ...
        byte[] excelAttach = processLoadMovementRepository.GenerateExcelToAttach(request.MasterDetailsFeedBack);
        ...
        var codeNotification = Environment.GetEnvironmentVariable("CODE_NOTIFICATION_MOVEMENT");
        ...
        if (Guid.TryParse(codeNotification, out Guid code))
        {
            ...
            processLoadMovementRepository.PutSftpFilesHistory(request.RepositoryServer, request.FeedBack, excelAttach);
            request.StepRecords.LastOrDefault().OutputJson = "Successful generation.";
