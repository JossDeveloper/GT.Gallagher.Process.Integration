...
public class SendNotificationHandler : Handler<ProcessLoadVehicleRequest>
{
    ...
    private readonly IProcessLoadVehicleRepository processLoadVehicleRepository;

    public SendNotificationHandler(ITPAServiceTokenRepository tokenRepository, INotificationEndProcessRepository notificationRepository, IProcessLoadVehicleRepository processLoadVehicleRepository)
    {
        this.tokenRepository = tokenRepository;
        this.notificationRepository = notificationRepository;
        this.processLoadVehicleRepository = processLoadVehicleRepository;
    }
    ...
    public override async Task ProcessRequest(ProcessLoadVehicleRequest request)
    {
        ...
        byte[] excelAttach = processLoadVehicleRepository.GenerateExcelToAttach(request.MasterDetailsFeedBack);
        ...
        var codeNotification = Environment.GetEnvironmentVariable("CODE_NOTIFICATION_VEHICLE");
        ...
        if (Guid.TryParse(codeNotification, out Guid code))
        {
            ...
            processLoadVehicleRepository.PutSftpFilesHistory(request.RepositoryServer, request.FeedBack, excelAttach);
            request.StepRecords.LastOrDefault().OutputJson = "Successful generation.";
