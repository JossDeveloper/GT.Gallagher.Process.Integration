...
public class SendNotificationHandler : Handler<ProcessLoadPolicyRequest>
{
    ...
    private readonly IProcessLoadPolicyRepository processLoadPolicyRepository;

    public SendNotificationHandler(ITPAServiceTokenRepository tokenRepository, INotificationEndProcessRepository notificationRepository, IProcessLoadPolicyRepository processLoadPolicyRepository)
    {
        this.tokenRepository = tokenRepository;
        this.notificationRepository = notificationRepository;
        this.processLoadPolicyRepository = processLoadPolicyRepository;
    }
    ...
    public override async Task ProcessRequest(ProcessLoadPolicyRequest request)
    {
        ...
        byte[] excelAttach = processLoadPolicyRepository.GenerateExcelToAttach(request.MasterDetailsFeedBack);
        ...
        var codeNotification = Environment.GetEnvironmentVariable("CODE_NOTIFICATION_POLICY");
        ...
        if (Guid.TryParse(codeNotification, out Guid code))
        {
            ...
            processLoadPolicyRepository.PutSftpFilesHistory(request.RepositoryServer, request.FeedBack, excelAttach);
            request.StepRecords.LastOrDefault().OutputJson = "Successful generation.";
