...
using Ent = GT.Gallagher.Process.Integration.Infrastructure.DataAccess.Entities.Process.LoadInsured;

namespace GT.Gallagher.Process.Integration.Infrastructure.DataAccess.Repositories.Process;

public class ProcessLoadInsuredRepository : IProcessLoadInsuredRepository
{
    private readonly IMapper _mapper;

    public ProcessLoadInsuredRepository(IMapper mapper) => _mapper = mapper;
    ...
    public int SaveProcessMaster(Guid masterCode, int userId, Guid processCode, Guid uniqueName, string fileName, string ext, List<ProcessLoadInsuredDetails> processLoadDetails, int quantityRows)
    {
        ...
        foreach (var singleRow in processLoadDetails)
        {
            masterDetails.Add(new EntCommon.MasterDetailsInput(singleRow.RowNumber,
                _mapper.Map<Ent.ProcessLoadInsured>(singleRow.ProcessLoadInsured),
    ...
    public List<ProcessLoadInsuredContent> BuildContentFile(List<(int, string)> lines)
    {
        List<ProcessLoadInsuredContent> result = new();
        int rowNumber = 2;
        ...
        using (var reader = new StringReader(string.Join('\n', lines.Select(item => item.Item2).ToList())))
        {
            ...
            {
                csv.Context.RegisterClassMap<ProcessLoadInsuredMap>();

                var records = csv.GetRecords<Ent.ProcessLoadInsured>().ToList();
                var lista = _mapper.Map<List<ProcessLoadInsured>>(records);
                    lista.ForEach(item => 
                    {
                        result.Add(new ProcessLoadInsuredContent(rowNumber++, item));
                    });
            }
        }
        return result;
    ...
    public List<ProcessLoadInsuredFeedBack> GetMasterDetailsFeedBack(Guid masterCode)
    {
        var parameters = new List<SqlParameter>()
        {
            new() { SqlDbType = SqlDbType.UniqueIdentifier, ParameterName = "@MasterCode", Direction = ParameterDirection.Input, Value = masterCode }
        ...
        var result = new Context().Set<Ent.ProcessLoadInsuredFeedBack>()
            .FromSqlRaw(@"exec SWP_Plot_Get_MasterDetailsInsured @MasterCode", parameters.ToArray())
            .ToList();

        return _mapper.Map<List<ProcessLoadInsuredFeedBack>>(result);
    }
    ...
    public int LoadDataToProcess(Guid masterCode)
    {
        ...
        new Context().Database.ExecuteSqlRaw(@"exec @returnValue = SWP_Plot_ProcessLoadInsured @MasterCode", parameters.ToArray());

        return (int) parameters[0].Value;
    }

    public int ProcessInsuredPlotData(Guid masterCode)
    {
        var parameters = new List<SqlParameter>()
        {
            new() { SqlDbType = SqlDbType.Int, ParameterName = "ReturnValue", Direction = ParameterDirection.Output },
        ...
        new Context().Database.ExecuteSqlRaw(@"exec @returnValue = SWP_Plot_RunProcess_Insured @MasterCode", parameters.ToArray());
    ...
    public byte[] GenerateExcelToAttach(List<ProcessLoadInsuredFeedBack> masterDetailsFeedBack)
