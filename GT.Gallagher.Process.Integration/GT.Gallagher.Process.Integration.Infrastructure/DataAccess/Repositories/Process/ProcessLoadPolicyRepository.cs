...
using Ent = GT.Gallagher.Process.Integration.Infrastructure.DataAccess.Entities.Process.LoadPolicy;

namespace GT.Gallagher.Process.Integration.Infrastructure.DataAccess.Repositories.Process;

public class ProcessLoadPolicyRepository : IProcessLoadPolicyRepository
{
    private readonly IMapper _mapper;

    public ProcessLoadPolicyRepository(IMapper mapper) => _mapper = mapper;
    ...
    public int SaveProcessMaster(Guid masterCode, int userId, Guid processCode, Guid uniqueName, string fileName, string ext, List<ProcessLoadPolicyDetails> processLoadDetails, int quantityRows)
    {
        ...
        foreach (var singleRow in processLoadDetails)
        {
            masterDetails.Add(new EntCommon.MasterDetailsInput(singleRow.RowNumber,
                _mapper.Map<Ent.ProcessLoadPolicy>(singleRow.ProcessLoadPolicy),
    ...
    public List<ProcessLoadPolicyContent> BuildContentFile(List<(int, string)> lines)
    {
        List<ProcessLoadPolicyContent> result = new();
        int rowNumber = 2;
        ...
        using (var reader = new StringReader(string.Join('\n', lines.Select(item => item.Item2).ToList())))
        {
            ...
            {
                csv.Context.RegisterClassMap<ProcessLoadPolicyMap>();

                var records = csv.GetRecords<Ent.ProcessLoadPolicy>().ToList();
                var lista = _mapper.Map<List<ProcessLoadPolicy>>(records);
                    lista.ForEach(item =>
                    {
                        result.Add(new ProcessLoadPolicyContent(rowNumber++, item));
                    });
            }
        }
        return result;
    ...
    public List<ProcessLoadPolicyFeedBack> GetMasterDetailsFeedBack(Guid masterCode)
    {
        var parameters = new List<SqlParameter>()
        {
            new() { SqlDbType = SqlDbType.UniqueIdentifier, ParameterName = "@MasterCode", Direction = ParameterDirection.Input, Value = masterCode }
        ...
        var result = new Context().Set<Ent.ProcessLoadPolicyFeedBack>()
            .FromSqlRaw(@"exec SWP_Plot_Get_MasterDetailsPolicy @MasterCode", parameters.ToArray())
            .ToList();

        return _mapper.Map<List<ProcessLoadPolicyFeedBack>>(result);
    }
    ...
    public int LoadDataToProcess(Guid masterCode)
    {
        ...
        new Context().Database.ExecuteSqlRaw(@"exec @returnValue = SWP_Plot_ProcessLoadPolicy @MasterCode", parameters.ToArray());

        return (int) parameters[0].Value;
    }

    public int ProcessPolicyPlotData(Guid masterCode)
    {
        var parameters = new List<SqlParameter>()
        {
            new() { SqlDbType = SqlDbType.Int, ParameterName = "ReturnValue", Direction = ParameterDirection.Output },
        ...
        new Context().Database.ExecuteSqlRaw(@"exec @returnValue = SWP_Plot_RunProcess_Policy @MasterCode", parameters.ToArray());
    ...
    public byte[] GenerateExcelToAttach(List<ProcessLoadPolicyFeedBack> masterDetailsFeedBack)
