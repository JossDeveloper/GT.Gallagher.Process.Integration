...
using Ent = GT.Gallagher.Process.Integration.Infrastructure.DataAccess.Entities.Process.LoadClient;

namespace GT.Gallagher.Process.Integration.Infrastructure.DataAccess.Repositories.Process;

public class ProcessLoadClientRepository : IProcessLoadClientRepository
{
    private readonly IMapper _mapper;

    public ProcessLoadClientRepository(IMapper mapper) => _mapper = mapper;
    ...
    public int SaveProcessMaster(Guid masterCode, int userId, Guid processCode, Guid uniqueName, string fileName, string ext, List<ProcessLoadClientDetails> processLoadDetails, int quantityRows)
    {
        ...
        foreach (var singleRow in processLoadDetails)
        {
            masterDetails.Add(new EntCommon.MasterDetailsInput(singleRow.RowNumber, 
                _mapper.Map<Ent.ProcessLoadClient>(singleRow.ProcessLoadClient),
    ...
    public List<ProcessLoadClientContent> BuildContentFile(List<(int, string)> lines)
    {
        List<ProcessLoadClientContent> result = new();
        int rowNumber = 2;
        ...
        using (var reader = new StringReader(string.Join('\n', lines.Select(item => item.Item2).ToList())))
        {
            ...
            {
                csv.Context.RegisterClassMap<ProcessLoadClientMap>();

                var records = csv.GetRecords<Ent.ProcessLoadClient>().ToList();
                var lista = _mapper.Map<List<ProcessLoadClient>>(records);
                    lista.ForEach(item =>
                    {
                        result.Add(new ProcessLoadClientContent(rowNumber++, item));
                    });
            }
        }
        return result;
    ...
    public List<ProcessLoadClientFeedBack> GetMasterDetailsFeedBack(Guid masterCode)
    {
        var parameters = new List<SqlParameter>()
        {
            new() { SqlDbType = SqlDbType.UniqueIdentifier, ParameterName = "@MasterCode", Direction = ParameterDirection.Input, Value = masterCode }
        ...
        var result = new Context().Set<Ent.ProcessLoadClientFeedBack>()
            .FromSqlRaw(@"exec SWP_Plot_Get_MasterDetailsClient @MasterCode", parameters.ToArray())
            .ToList();

        return _mapper.Map<List<ProcessLoadClientFeedBack>>(result);
    }
    ...
    public int LoadDataToProcess(Guid masterCode)
    {
        ...
        new Context().Database.ExecuteSqlRaw(@"exec @returnValue = SWP_Plot_ProcessLoadClient @MasterCode", parameters.ToArray());

        return (int) parameters[0].Value;
    }

    public int ProcessClientPlotData(Guid masterCode)
    {
        var parameters = new List<SqlParameter>()
        {
            new() { SqlDbType = SqlDbType.Int, ParameterName = "ReturnValue", Direction = ParameterDirection.Output },
        ...
        new Context().Database.ExecuteSqlRaw(@"exec @returnValue = SWP_Plot_RunProcess_Client @MasterCode", parameters.ToArray());
    ...
    public byte[] GenerateExcelToAttach(List<ProcessLoadClientFeedBack> masterDetailsFeedBack)
