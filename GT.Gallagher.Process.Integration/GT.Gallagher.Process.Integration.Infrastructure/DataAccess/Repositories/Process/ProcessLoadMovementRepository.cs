...
using Ent = GT.Gallagher.Process.Integration.Infrastructure.DataAccess.Entities.Process.LoadMovement;

namespace GT.Gallagher.Process.Integration.Infrastructure.DataAccess.Repositories.Process;

public class ProcessLoadMovementRepository : IProcessLoadMovementRepository
{
    private readonly IMapper _mapper;

    public ProcessLoadMovementRepository(IMapper mapper) => _mapper = mapper;
    ...
    public int SaveProcessMaster(Guid masterCode, int userId, Guid processCode, Guid uniqueName, string fileName, string ext, List<ProcessLoadMovementDetails> processLoadDetails, int quantityRows)
    {
        ...
        foreach (var singleRow in processLoadDetails)
        {
            masterDetails.Add(new EntCommon.MasterDetailsInput(singleRow.RowNumber,
                _mapper.Map<Ent.ProcessLoadMovement>(singleRow.ProcessLoadMovement),
    ...
    public List<ProcessLoadMovementContent> BuildContentFile(List<(int, string)> lines)
    {
        List<ProcessLoadMovementContent> result = new();
        int rowNumber = 2;
        ...
        using (var reader = new StringReader(string.Join('\n', lines.Select(item => item.Item2).ToList())))
        {
            ...
            {
                csv.Context.RegisterClassMap<ProcessLoadMovementMap>();

                var records = csv.GetRecords<Ent.ProcessLoadMovement>().ToList();
                var lista = _mapper.Map<List<ProcessLoadMovement>>(records);
                    lista.ForEach(item =>
                    {
                        result.Add(new ProcessLoadMovementContent(rowNumber++, item));
                    });
            }
        }
        return result;
    ...
    public List<ProcessLoadMovementFeedBack> GetMasterDetailsFeedBack(Guid masterCode)
    {
        var parameters = new List<SqlParameter>()
        {
            new() { SqlDbType = SqlDbType.UniqueIdentifier, ParameterName = "@MasterCode", Direction = ParameterDirection.Input, Value = masterCode }
        ...
        var result = new Context().Set<Ent.ProcessLoadMovementFeedBack>()
            .FromSqlRaw(@"exec SWP_Plot_Get_MasterDetailsMovement @MasterCode", parameters.ToArray())
            .ToList();

        return _mapper.Map<List<ProcessLoadMovementFeedBack>>(result);
    }
    ...
    public int LoadDataToProcess(Guid masterCode)
    {
        ...
        new Context().Database.ExecuteSqlRaw(@"exec @returnValue = SWP_Plot_ProcessLoadMovement @MasterCode", parameters.ToArray());

        return (int) parameters[0].Value;
    }

    public int ProcessMovementPlotData(Guid masterCode)
    {
        var parameters = new List<SqlParameter>()
        {
            new() { SqlDbType = SqlDbType.Int, ParameterName = "ReturnValue", Direction = ParameterDirection.Output },
        ...
        new Context().Database.ExecuteSqlRaw(@"exec @returnValue = SWP_Plot_RunProcess_Movement @MasterCode", parameters.ToArray());
    ...
    public byte[] GenerateExcelToAttach(List<ProcessLoadMovementFeedBack> masterDetailsFeedBack)
